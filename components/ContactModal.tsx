  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4 transition-opacity duration-300 ease-in-out" onClick={onClose} >
      <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md transform transition-all duration-300 ease-in-out scale-95 opacity-0 animate-modalShow" onClick={(e) => e.stopPropagation()} >
        ...
        <p className="text-slate-600 mb-6"> To get started, please fill out my intake form. This will help me understand your needs better before our chat. I&apos;ll get back to you as soon as possible to discuss how I can best support your business. </p>
        <a href={intakeFormUrl} target="_blank" rel="noopener noreferrer" className="w-full inline-block text-center bg-cyan-700 hover:bg-cyan-800 text-white font-semibold py-3 px-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 ease-in-out transform hover:scale-105 text-base sm:text-lg" > Go to Intake Form </a>
        <button onClick={onClose} className="w-full mt-3 text-sm text-slate-600 hover:text-slate-800 py-2 rounded-md transition-colors" > Or, keep Browse </button>
      </div>
